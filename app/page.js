...
export default function Page() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mounted && sdk) {
          // Keep sdk object if provides functionality
          sdkRef.current = sdk;
          console.log("Farcaster SDK loaded:", sdk);
        }
      } catch (err) {
        // SDK missing or not available in this environment -> safe to ignore
  ...
  async function shareScore() {
    ...
    // 1) Try Farcaster SDK custom method if present (not all SDKs expose same API)
    try {
      ...
      // If SDK exposes a different API, the developer can replace above line.
    } catch (err) {
      console.warn("Farcaster share failed:", err);
    }

    // 2) Fallback to Web Share API (mobile)
    if (navigator.share) {
      try {
